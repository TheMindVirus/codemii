...
def my_repr(b, start=''):
    special = lambda x: type(x) not in (int, str)
    if type(b) == dict:
        if sum(1 for i in filter(special, b.values())) == 0:
            rep = '{ '
            f = False
        ...
        if sum(1 for i in (filter(special, b))) == 0:
...
def unnullterm(var, size):
    ...
    return var.ljust(size) #, b' ')
	
def untv(name):
    #assert('\0' not in name)
    lex = len(name) + 16
    lex -= lex % 16
    return name.ljust(lex) #, b' ')
...
def parse_data(chunk, definition, prefix=None, start=0):
    ...
    for typ, name, _ in df:
        try:
            ...
            if typ == 'char':
                var = chunk[pos:pos+size].rstrip(chr(0).encode())
                pos += size
            else:
                var = []
                for n in range(size):
