...
for animtype, elname, animcat, animents in anims:
    ...
    a = (animtype + chr(len(animents)) + '\0\0\0').encode()
    csb = b''
    for c in cs:
        a += struct.pack('>I', 8 + 4*len(cs) + len(csb)) # offset
        csb += c
    a += csb
    _ = unnullterm(elname, 0x14).encode() + struct.pack('>II', animcat, len(ab) + 0x1c*(len(anims) - i))
...
rlan = b'RLAN\xfe\xff\x00\x08\x00\x00\x00\x00' # size later
rlan += struct.pack('>hh', 0x10, 1)

rlan = rlan[:8] + struct.pack('>I', len(rlan)) + rlan[0xc:]

rlan += b'pai1' + struct.pack('>I', len(pai)+8) + pai
