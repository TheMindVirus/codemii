...
def main():
    ...
    assert(rlyt[0:8] == b'RLYT\xfe\xff\x00\x08')
    ...
    for typ, chunk in ch:
        ...
        if typ in ('pic1', 'pan1', 'bnd1', 'wnd1', 'txt1'):
            ...
            #print(chunk[:pos].replace(b' ', b'\0'), unparse_data(vars, 'pane').replace(b' ', b'\0'))
            assert(chunk[:pos].replace(b' ', b'\0') == unparse_data(vars, 'pane').replace(b' ', b'\0')) # for testing
            if typ == 'txt1':
                ...
                vars2['~pic.texcoords'] = []
                for n in range(vars2['~pic.num_texcoords']):
